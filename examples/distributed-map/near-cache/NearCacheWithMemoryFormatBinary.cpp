...
int main() {
    ...
    hazelcast::util::SharedPtr<config::NearCacheConfig<int, std::string> > nearCacheConfig(
    ...
    hazelcast::util::SharedPtr<std::string> firstGet = map.get(1);
    // the second and third get() will be served from the Near Cache
    hazelcast::util::SharedPtr<std::string> secondGet = map.get(1);
    hazelcast::util::SharedPtr<std::string> thirdGet = map.get(1);
