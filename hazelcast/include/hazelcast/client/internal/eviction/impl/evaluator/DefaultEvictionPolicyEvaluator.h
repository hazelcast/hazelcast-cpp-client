...
class DefaultEvictionPolicyEvaluator
  : public EvictionPolicyEvaluator<MAPKEY, MAPVALUE, A, E>
{
public:
    ...
    /**
     * The evaluate method implements the {@link
     * com.hazelcast.config.EvictionPolicy} rule on the given input set of
     * candidates.
     *
     * @param evictionCandidates Multiple {@link
     * com.hazelcast.internal.eviction.EvictionCandidate} to be evicted
     *
     * @return multiple {@link
     * com.hazelcast.internal.eviction.EvictionCandidate} these are available to
     * be evicted
     */
    ...
    evaluate(util::Iterable<EvictionCandidate<MAPKEY, MAPVALUE, A, E>>&
               eviction_candidates) const override
    {
        std::shared_ptr<eviction::EvictionCandidate<MAPKEY, MAPVALUE, A, E>>
        ...
        while (iterator->has_next()) {
            ...
            if (selectedEvictionCandidate.get() == NULL) {
                ...
                std::shared_ptr<E> evictable =
                  currentEvictionCandidate->get_evictable();
                ...
                int comparisonResult = eviction_policy_comparator_->compare(
                  selectedEvictionCandidate.get(),
                  currentEvictionCandidate.get());
                if (comparisonResult ==
                    EvictionPolicyComparator<A, E>::
...
private:
        ...
        eviction_candidate) const
    {
        if (eviction_candidate.get() == NULL) {
            ...
            std::unique_ptr<std::vector<std::shared_ptr<
              eviction::EvictionCandidate<MAPKEY, MAPVALUE, A, E>>>>
              result(new std::vector<std::shared_ptr<
                       eviction::EvictionCandidate<MAPKEY, MAPVALUE, A, E>>>());
