...
protected:
    ...
    {
        client_ =
          std::unique_ptr<hazelcast_client>(new hazelcast_client{ new_client(get_config()).get() });
    ...
    void create_near_cache_context()
    {
        ...
        near_cached_client_ = std::unique_ptr<hazelcast_client>(
          new hazelcast_client{ new_client(std::move(near_cached_client_config_)).get() });
...
{
    ASSERT_NO_THROW(topic_ =
                      client->get_reliable_topic("removeMessageListener_whenNonExisting").get());
...
TEST_F(IssueTest, testIssue753)
{
    ...
    auto result = map->get<int, int>(1)
                    .then(boost::launch::async,
                          [](boost::future<boost::optional<int>> f) { return 3 * f.get().value(); })
                    .get();
...
std::string
ClientTestSupport::get_test_name()
{
