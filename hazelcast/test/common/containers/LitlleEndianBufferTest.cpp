...
#include "common/containers/LitlleEndianBufferTest.h"
        ...
        namespace test {
            namespace common {
                namespace containers {
                    ...
                    LittleEndianBufferTest::LittleEndianBufferTest()
                            : iTestFixture<LittleEndianBufferTest>("LittleEndianBufferTest") {
                    }
                    ...
                    void LittleEndianBufferTest::beforeClass() {
                    }

                    void LittleEndianBufferTest::afterClass() {
                    }

                    void LittleEndianBufferTest::beforeTest() {
                    ...
                    void LittleEndianBufferTest::testBinaryFormat() {
                        ...
                        set((uint8_t) 0x8A);
                        ASSERT_EQUAL(0x8A, writeBuffer[0]);
                        
                        set(true);
                        ASSERT_EQUAL(0x01, writeBuffer[1]);

                        set(false);
                        ASSERT_EQUAL(0x00, writeBuffer[2]);

                        set('C');
                        ASSERT_EQUAL('C', writeBuffer[3]);

                        int16_t int16Val = 0x7BCD;
                        set(int16Val);
                        ASSERT_EQUAL(0xCD, writeBuffer[4]);
                        ASSERT_EQUAL(0x7B, writeBuffer[5]);

                        uint16_t uInt16Val = 0xABCD;
                        set(uInt16Val);
                        ASSERT_EQUAL(0xCD, writeBuffer[6]);
                        ASSERT_EQUAL(0xAB, writeBuffer[7]);

                        int32_t int32Val = 0xAEBCEEFF;
                        set(int32Val);
                        ASSERT_EQUAL(0xFF, writeBuffer[8]);
                        ASSERT_EQUAL(0xEE, writeBuffer[9]);
                        ASSERT_EQUAL(0xBC, writeBuffer[10]);
                        ASSERT_EQUAL(0xAE, writeBuffer[11]);


                        set(std::string("Test Data"));
                        ASSERT_EQUAL(0x09, (int)writeBuffer[12]);
                        ASSERT_EQUAL(0x0, writeBuffer[13]);
                        ASSERT_EQUAL(0x0, writeBuffer[14]);
                        ASSERT_EQUAL(0x0, writeBuffer[15]);
                        ASSERT_EQUAL('T', writeBuffer[16]);
