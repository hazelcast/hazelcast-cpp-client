...
ClientProxy::destroy_remotely()
{
    ...
    return spi::impl::ClientInvocation::create(
             ...
             get_name())
      ->invoke()
      .then(boost::launch::sync, [](boost::future<protocol::ClientMessage> f) { f.get(); });
}
...
void
ClientInvocation::set_exception(const std::exception& e, boost::exception_ptr exception_ptr)
{
  ...
  const partition_table& current)
{
    ...
    if (!current.connection_id || connection_id != current.connection_id) {
        HZ_LOG(lg, finest, ([&current, connection_id]() {
                   auto frmt =
                     boost::format("Event coming from a new connection. Old connection id: %1%, "
                                   "new connection %2%");
...
void
ClientPartitionServiceImpl::log_failure(int32_t connection_id,
                                        int32_t version,
...
void
cluster_view_listener::try_reregister_to_random_connection(int32_t old_connection_id)
{
...
cluster_view_listener::event_handler::on_listener_register()
{
    ...
    HZ_LOG(
      ...
      finest,
      boost::str(
        boost::format("Registered cluster_view_listener::event_handler to connection with id %1%") %
        connection_id));
}
...
#ifdef HZ_BUILD_WITH_SSL
    try {
        util::SyncHttpsClient httpsConnection(
          cloud_base_url_, std::string(CLOUD_URL_PATH) + cloud_config_.discovery_token, timeout_);
        ...
        std::throw_with_nested(boost::enable_current_exception(
          exception::illegal_state("cloud_discovery::get_addresses", e.what())));
    }
#else
