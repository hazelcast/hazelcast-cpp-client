...
hazelcast_client_instance_impl::create_address_provider()
{
    ...
    if (cloud_enabled) {
        auto cloud_provider = std::make_shared<spi::impl::discovery::cloud_discovery>(
          cloudConfig,
    ...
    if (awsDiscoveryEnabled) {
        ...
        return std::unique_ptr<connection::AddressProvider>(
          new spi::impl::discovery::remote_address_provider(
            [=]() { return aws_addr_provider->get_addresses(); }, !awsConfig.is_inside_aws()));
    }
    ...
    if (nearCacheConfig) {
        return proxy_manager_
          ...
          .then(boost::launch::sync,
                [=](boost::shared_future<
                    std::shared_ptr<map::NearCachedClientMapProxy<serialization::pimpl::data,
...
{
    return boost::async(
      [](client::client_config&& c) { return client::hazelcast_client(std::move(c)); },
      std::move(config));
}
} // namespace hazelcast
