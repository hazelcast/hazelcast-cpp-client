...
/**
 * Provides a context to do transactional operations; so beginning/committing
 * transactions, but also retrieving transactional data-structures like the
 * TransactionalMap.
 *
...
public:
     ...
     *
     * @throws transaction if no transaction is active or the transaction could
     * not be committed.
     */
    ...
    /**
     * Returns the transactional distributed map instance with the specified
     * name.
     *
     *
    ...
    {
        return get_transactional_object<transactional_map>(imap::SERVICE_NAME,
                                                           name);
    }
    ...
    {
        return get_transactional_object<transactional_queue>(
          iqueue::SERVICE_NAME, name);
    }
     ...
     */
    std::shared_ptr<transactional_multi_map> get_multi_map(
      const std::string& name)
    {
        return get_transactional_object<transactional_multi_map>(
          multi_map::SERVICE_NAME, name);
    }
    ...
    {
        return get_transactional_object<transactional_list>(ilist::SERVICE_NAME,
                                                            name);
    }
    ...
    {
        return get_transactional_object<transactional_set>(iset::SERVICE_NAME,
                                                           name);
    }
                                                ...
                                                const std::string& name)
    {
        ...
        std::shared_ptr<T> obj =
          std::static_pointer_cast<T>(txn_object_map_.get(key));
        if (!obj) {
...
private:
    ...
    util::SynchronizedMap<std::pair<std::string, std::string>,
                          proxy::TransactionalObject>
      txn_object_map_;
};
} // namespace client
} // namespace hazelcast
