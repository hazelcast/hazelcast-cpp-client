...
class BaseHeapNearCacheRecordStore
  : public AbstractNearCacheRecordStore<K,
                                        V,
                                        KS,
                                        R,
                                        HeapNearCacheRecordMap<K, V, KS, R>>
{
public:
    typedef AbstractNearCacheRecordStore<K,
                                         V,
                                         KS,
                                         R,
                                         HeapNearCacheRecordMap<K, V, KS, R>>
    ...
    {
        std::vector<std::pair<std::shared_ptr<KS>, std::shared_ptr<R>>>
...
protected:
    std::unique_ptr<eviction::MaxSizeChecker>
    ...
    {
        typename client::config::eviction_config::max_size_policy
          maxSizePolicy = eviction_config.get_maximum_size_policy();
        ...
        out << "Invalid max-size policy " << '(' << (int)maxSizePolicy
            << ") for " << near_cache_config.get_name() << "! Only "
            << (int)client::config::eviction_config::ENTRY_COUNT
            << " is supported.";
