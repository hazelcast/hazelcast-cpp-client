...
struct PartitionAwareString
  : public hazelcast::client::partition_aware<std::string>
{
...
const std::string PartitionAwareString::desiredPartitionString =
  "desiredKeyString";
...
main()
{
    ...
    // Puts the key, value tokyo at the partition for "desiredKeyString" rather
    // than the partition for "MyString"
    map->put<PartitionAwareString, std::string>(partitionKey, "Tokyo").get();
    auto value =
      map->get<PartitionAwareString, std::string>(partitionKey).get();
