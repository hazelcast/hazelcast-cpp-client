...
{
public:
    typedef std::vector<std::pair<boost::uuids::uuid, int64_t>>
      timestamp_vector;
...
private:
     ...
     * @return false for the pair.first if timestamp does not exist for
     * replicaId, otherwise returns true for pair.first and the timestamp of the
     * replica as the pair.second.
     */
    std::pair<bool, int64_t> get_timestamp_for_replica(
