...
SyncHttpsClient::connect_and_get_response()
{
    ...
    try {
        ...
        boost::asio::async_connect(
          ...
          addresses,
          [&](const boost::system::error_code& ec, const boost::asio::ip::tcp::endpoint& endpoint) {
              error = ec;
          });
        ...
        boost::asio::async_write(
          *socket_, request.data(), [&](const boost::system::error_code& ec, std::size_t) {
              error = ec;
          });
        run(error, timeout_);
        ...
        throw client::exception::io(
          ...
          (boost::format("Could not retrieve response from https://%1%%2%. Error:%3%") % server_ %
           uri_path_ % e.what())
            .str());
    }
...
void
SyncHttpsClient::run(boost::system::error_code& error, std::chrono::steady_clock::duration timeout)
{
...
BackoffIdleStrategy::park_time(int64_t n) const
{
    ...
    return proposedShift > max_shift_
             ? max_park_period_ns_
