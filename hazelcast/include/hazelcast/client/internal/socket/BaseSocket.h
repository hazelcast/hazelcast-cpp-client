...
public:
        ...
        socket_strand_.post([=]() {
            ...
            if (datas.size() == 1) {
                boost::asio::async_write(
                  socket_, boost::asio::buffer(datas[0]), socket_strand_.wrap(handler));
            } else {
    ...
    boost::asio::io_context::strand& get_executor() noexcept override { return socket_strand_; }
...
protected:
    ...
    void do_read(const std::shared_ptr<connection::Connection> connection)
    {
        socket_.async_read_some(
          ...
          socket_strand_.wrap([=](const boost::system::error_code& ec, std::size_t bytes_read) {
              if (ec) {
