...
main()
{
    ...
    NearCacheSupport::print_near_cache_stats(
      map,
      "The noNearCacheMap->put(key, \"foo\")) call has no effect on the Near "
      "Cache of map");
    ...
    NearCacheSupport::print_near_cache_stats(
      map,
      "The noNearCacheMap->put(key, \"foo\")) call will invalidate the Near "
      "Cache of map");

    NearCacheSupport::wait_for_invalidation_events();
    NearCacheSupport::print_near_cache_stats(
      map,
      "The Near Cache of map is empty after the invalidation event has been "
      "processed");
