...
public:
     ...
     */
    template<typename T>
    static const T& check_positive(const T& value,
                                   const std::string& error_message)
    {
        if (value <= 0) {
            throw client::exception::illegal_argument(
              "Preconditions::checkPositive", error_message);
        }
        return value;
    }
     ...
     */
    template<typename T>
    static const std::shared_ptr<T>& check_not_null(
    ...
    template<typename T>
    static const T* check_not_null(const T* argument,
                                   const std::string& error_message)
    {
        if (!argument) {
    ...
    static void check_not_nill(boost::uuids::uuid id,
                               const std::string& error_message)
    {
        if (id.is_nil()) {
            throw client::exception::null_pointer(error_message);
    ...
    /**
     * Tests if an argument is not empty. The argument should have the size
     * method.
     *
     ...
     */
    template<typename T>
    static void check_not_empty(const T& argument,
                                const std::string& error_message)
    {
        if (argument.size() == 0) {
            throw client::exception::illegal_argument(error_message);
     ...
     */
    template<typename T>
    static const std::shared_ptr<T>& is_not_null(
      const std::shared_ptr<T>& argument,
     ...
     */
    template<typename T>
    static const T& check_not_negative(const T& value,
                                       const std::string& error_message)
    {
        if (value < 0) {
            throw client::exception::illegal_argument(
              "Preconditions::checkNotNegative", error_message);
        }
        return value;
    }
     ...
     * @param variableName      The neame of the variable to be printed in the
     * exception message
                          ...
                          const std::string& variable_name)
    {
        if (actual_size > expected_maximum) {
            throw(
              client::exception::exception_builder<
                client::exception::illegal_argument>("Preconditions::check_max")
              << variable_name << "(" << actual_size << ")"
              << " can't be larger than " << expected_maximum)
              .build();
        }
    }
    ...
    /**
     * @throws client::exception::invalid_configuration if the user does not
     * compile with HZ_BUILD_WITH_SSL flag but is trying to use a feature (e.g.
     * TLS, AWS Cloud Discovery) that needs this flag.
     */
     ...
     * @param expression   the expression tested to see if it is {@code true}.
     * @param errorMessage the errorMessage
     * @throws client::Exception::illegal_argument if the supplied expression is
     * {@code false}.
     */
