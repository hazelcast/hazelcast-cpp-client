...
if exist "hazelcast-remote-controller-%HAZELCAST_RC_VERSION%.jar" (
    ...
    call mvn -X dependency:2.10:get -Dtransitive=false -DremoteRepositories=%SNAPSHOT_REPO% -Dartifact=com.hazelcast:hazelcast-remote-controller:%HAZELCAST_RC_VERSION% -Ddest=hazelcast-remote-controller-%HAZELCAST_RC_VERSION%.jar || (
...
if exist  "hazelcast-%HAZELCAST_TEST_VERSION%-tests.jar" (
    ...
    call mvn -X dependency:2.10:get -Dtransitive=false -DremoteRepositories=%SNAPSHOT_REPO% -Dartifact=com.hazelcast:hazelcast:%HAZELCAST_TEST_VERSION%:jar:tests -Ddest=hazelcast-%HAZELCAST_TEST_VERSION%-tests.jar || (
...
if exist  "hazelcast-sql-%HZ_VERSION%.jar" (
    ...
    call mvn -X dependency:2.10:get -Dtransitive=false -DremoteRepositories=%SNAPSHOT_REPO% -Dartifact=com.hazelcast:hazelcast-sql:%HZ_VERSION%:jar -Ddest=hazelcast-sql-%HZ_VERSION%.jar || (
...
if exist "hazelcast-enterprise-%HAZELCAST_ENTERPRISE_VERSION%.jar" (
    ...
    call mvn -X dependency:2.10:get -Dtransitive=false -DremoteRepositories=%ENTERPRISE_REPO% -Dartifact=com.hazelcast:hazelcast-enterprise:%HAZELCAST_ENTERPRISE_VERSION% -Ddest=hazelcast-enterprise-%HAZELCAST_ENTERPRISE_VERSION%.jar || (
