...
/**
 * Interface for evaluation implementations of {@link
...
public:
    ...
    /**
     * The evaluate method implements the actual policy rules and is called on
     * every eviction to select one or more candidates to be evicted from the
     * given input set of candidates. The selection algorithm should execute in
     * a constant time to deliver a predictable timing results of the eviction
     * system.
     *
     * @param evictionCandidates Multiple {@link EvictionCandidate} to be
     * evicted
     *
     * @return multiple {@link EvictionCandidate}s that are available to be
     * evicted
     */
