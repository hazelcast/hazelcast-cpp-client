...
namespace eviction {
/**
 * Provider to get any kind ({@link EvictionPolicyType}) of {@link
...
public:
    /**
     * Gets the {@link EvictionPolicyEvaluator} implementation specified with
     * {@code evictionPolicy}.
     *
    ...
    get_eviction_policy_evaluator(
      const client::config::eviction_config& eviction_config)
    {

        std::shared_ptr<EvictionPolicyComparator<MAPKEY, MAPVALUE>>
          evictionPolicyComparator;

        auto evictionPolicyType = eviction_config.get_eviction_policy();
        evictionPolicyComparator =
          create_eviction_policy_comparator<MAPKEY, MAPVALUE>(
            evictionPolicyType);

        return std::unique_ptr<EvictionPolicyEvaluator<MAPKEY, MAPVALUE, A, E>>(
          new impl::evaluator::
            DefaultEvictionPolicyEvaluator<MAPKEY, MAPVALUE, A, E>(
              evictionPolicyComparator));
    }

private:
    template<typename A, typename E>
    static std::shared_ptr<EvictionPolicyComparator<A, E>>
    create_eviction_policy_comparator(
      ::hazelcast::client::config::eviction_policy eviction_policy)
    {
        switch (eviction_policy) {
            ...
            default:
                ...
                out << "Unsupported eviction policy type: "
                    << (int)eviction_policy;
