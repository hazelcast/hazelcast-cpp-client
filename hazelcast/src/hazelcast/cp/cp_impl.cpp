...
proxy_session_manager::request_new_session(const raft_group_id& group_id)
{
    ...
    auto response = client::spi::impl::ClientInvocation::create(client_, request, "sessionManager")
                      ->invoke()
                      .get();
...
proxy_session_manager::heartbeat(const raft_group_id& group_id, int64_t session_id)
{
    ...
    return client::spi::impl::ClientInvocation::create(client_, request, "sessionManager")
      ->invoke();
}
