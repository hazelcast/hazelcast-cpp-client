...
public:
    ...
    void release_session(const raft_group_id& group_id,
                         int64_t session_id,
                         int32_t count);
    ...
    // for testing
    int64_t get_session_acquire_count(const raft_group_id& group_id,
                                      int64_t session_id);
...
private:
    ...
    boost::future<client::protocol::ClientMessage> heartbeat(
      const raft_group_id& group_id,
      int64_t session_id);

    boost::future<client::protocol::ClientMessage> close_session(
      const raft_group_id& group_id,
      int64_t session_id);
