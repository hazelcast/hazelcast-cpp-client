...
protected:
    client_config new_ssl_client_config(bool set_different_protocol) override
    {
        ...
        boost::asio::ssl::context::method method =
          set_different_protocol ? boost::asio::ssl::context::method::sslv23
...
{
    return socket_interceptor().on_connect([&l](const hazelcast::client::socket& connected_sock) {
...
TEST_F(BasicPnCounterAPITest, testGetStart)
{
    std::shared_ptr<pn_counter> pnCounter =
      client->get_pn_counter(testing::UnitTest::GetInstance()->current_test_info()->name()).get();
...
TEST_F(ClientPNCounterNoDataMemberTest, noDataMemberExceptionIsThrown)
{
    ...
    auto pnCounter =
      hz.get_pn_counter(testing::UnitTest::GetInstance()->current_test_info()->name()).get();
...
protected:
    virtual void SetUp()
    {
        ...
        flake_id_generator_ =
          client
            ->get_flake_id_generator(testing::UnitTest::GetInstance()->current_test_info()->name())
            .get();
    }
...
TEST_F(ClientTxnMapTest, testPutGet)
{
    ...
    ASSERT_EQ("value1",
              (client_.get_map(name).get()->get<std::string, std::string>("key1").get().value()));
}
...
{
    std::vector<byte> bytes{ 'a', 'b' };
    serialization::pimpl::data_input<std::vector<byte>> dataInput(boost::endian::order::little,
                                                                  bytes);
