...
struct CompactRabinFingerprintTest : public ::testing::Test
{
    ...
    void check_each(std::vector<entry_t<T>> entries)
    {
        ...
        {
            using namespace hazelcast::client::serialization::pimpl::rabin_finger_print;

            auto fp_before = std::get<0>( e );
            auto value     = std::get<1>( e );
            auto expected  = std::get<2>( e );
            auto fp_after  = fingerprint64(fp_before , value);
...
TEST_F(CompactRabinFingerprintTest, test_schema)
{
    ...
    auto writer = hazelcast::client::serialization::pimpl::create_compact_writer(&s_writer);

    writer.write_int32("id", 0);
    writer.write_string("name" , std::string {} );
...
TEST_F(CompactSchemaTest, test_constructor)
{
    ...
    schema sch
    {
        ...
        {
            { std::string { "boolean-0"                 } , field_descriptor { BOOLEAN                          } } ,
            { std::string { "boolean-1"                 } , field_descriptor { BOOLEAN                          } } ,
            { std::string { "boolean-2"                 } , field_descriptor { BOOLEAN                          } } ,
            { std::string { "boolean-3"                 } , field_descriptor { BOOLEAN                          } } ,
            { std::string { "boolean-4"                 } , field_descriptor { BOOLEAN                          } } ,
            { std::string { "boolean-5"                 } , field_descriptor { BOOLEAN                          } } ,
            { std::string { "boolean-6"                 } , field_descriptor { BOOLEAN                          } } ,
            { std::string { "boolean-7"                 } , field_descriptor { BOOLEAN                          } } ,
            { std::string { "boolean-8"                 } , field_descriptor { BOOLEAN                          } } ,
            { std::string { "boolean-9"                 } , field_descriptor { BOOLEAN                          } } ,
            { std::string { "boolean[]"                 } , field_descriptor { ARRAY_OF_BOOLEAN                 } } ,
            { std::string { "int8"                      } , field_descriptor { INT8                             } } ,
            { std::string { "int8[]"                    } , field_descriptor { ARRAY_OF_INT8                    } } ,
            { std::string { "int16"                     } , field_descriptor { INT16                            } } ,
            { std::string { "int16[]"                   } , field_descriptor { ARRAY_OF_INT16                   } } ,
            { std::string { "int32"                     } , field_descriptor { INT32                            } } ,
            { std::string { "int32[]"                   } , field_descriptor { ARRAY_OF_INT32                   } } ,
            { std::string { "int64"                     } , field_descriptor { INT64                            } } ,
            { std::string { "int64[]"                   } , field_descriptor { ARRAY_OF_INT64                   } } ,
            { std::string { "float32"                   } , field_descriptor { FLOAT32                          } } ,
            { std::string { "float32[]"                 } , field_descriptor { ARRAY_OF_FLOAT32                 } } ,
            { std::string { "float64"                   } , field_descriptor { FLOAT64                          } } ,
            { std::string { "float64[]"                 } , field_descriptor { ARRAY_OF_FLOAT64                 } } ,
            { std::string { "string"                    } , field_descriptor { STRING                           } } ,
            { std::string { "string[]"                  } , field_descriptor { ARRAY_OF_STRING                  } } ,
            { std::string { "decimal"                   } , field_descriptor { DECIMAL                          } } ,
            { std::string { "decimal[]"                 } , field_descriptor { ARRAY_OF_DECIMAL                 } } ,
            { std::string { "time"                      } , field_descriptor { TIME                             } } ,
            { std::string { "time[]"                    } , field_descriptor { ARRAY_OF_TIME                    } } ,
            { std::string { "date"                      } , field_descriptor { DATE                             } } ,
            { std::string { "date[]"                    } , field_descriptor { ARRAY_OF_DATE                    } } ,
            { std::string { "timestamp"                 } , field_descriptor { TIMESTAMP                        } } ,
            { std::string { "timestamp[]"               } , field_descriptor { ARRAY_OF_TIMESTAMP               } } ,
            { std::string { "timestamp_with_timezone"   } , field_descriptor { TIMESTAMP_WITH_TIMEZONE          } } ,
            { std::string { "timestamp_with_timezone[]" } , field_descriptor { ARRAY_OF_TIMESTAMP_WITH_TIMEZONE } } ,
            { std::string { "compact"                   } , field_descriptor { COMPACT                          } } ,
            { std::string { "compact[]"                 } , field_descriptor { ARRAY_OF_COMPACT                 } } ,
            { std::string { "nullable<boolean>"         } , field_descriptor { NULLABLE_BOOLEAN                 } } ,
            { std::string { "nullable<boolean>[]"       } , field_descriptor { ARRAY_OF_NULLABLE_BOOLEAN        } } ,
            { std::string { "nullable<int8>"            } , field_descriptor { NULLABLE_INT8                    } } ,
            { std::string { "nullable<int8>[]"          } , field_descriptor { ARRAY_OF_NULLABLE_INT8           } } ,
            { std::string { "nullable<int16>"           } , field_descriptor { NULLABLE_INT16                   } } ,
            { std::string { "nullable<int16>[]"         } , field_descriptor { ARRAY_OF_NULLABLE_INT16          } } ,
            { std::string { "nullable<int32>"           } , field_descriptor { NULLABLE_INT32                   } } ,
            { std::string { "nullable<int32>[]"         } , field_descriptor { ARRAY_OF_NULLABLE_INT32          } } ,
            { std::string { "nullable<int64>"           } , field_descriptor { NULLABLE_INT64                   } } ,
            { std::string { "nullable<int64>[]"         } , field_descriptor { ARRAY_OF_NULLABLE_INT64          } } ,
            { std::string { "nullable<float32>"         } , field_descriptor { NULLABLE_FLOAT32                 } } ,
            { std::string { "nullable<float32>[]"       } , field_descriptor { ARRAY_OF_NULLABLE_INT64          } } ,
            { std::string { "nullable<float64>"         } , field_descriptor { NULLABLE_FLOAT64                 } } ,
            { std::string { "nullable<float64>[]"       } , field_descriptor { ARRAY_OF_NULLABLE_FLOAT64        } }
        }
    };

    auto result = sch.fields();

    // Assert num of fields
    ASSERT_EQ( sch.fixed_size_fields_length()  , 29 );
    ASSERT_EQ( sch.number_of_var_size_fields() , 35 );
...
TEST_F(CompactSchemaTest, test_with_no_fields)
{
    ...
    ASSERT_EQ( sch.fields().size()            , 0 );
    ASSERT_EQ( sch.fixed_size_fields_length() , 0 );
    ASSERT_EQ( sch.number_of_var_size_fields(), 0 );
}
...
TEST_F(CompactSerializationTest, test_read_when_field_does_not_exist)
{
    ...
    ASSERT_THROW(
        ss.to_object<wrong_field_name_read_obj>(data),
...
TEST_F(CompactSerializationTest, test_read_with_type_mismatch)
{
    ...
    auto data = ss.to_data(obj);
    ASSERT_THROW(
        ss.to_object<type_mistmatch_obj>(data) ,
        exception::hazelcast_serialization
    );
}
...
template<>
struct hz_serializer<compact::test::wrong_field_name_read_obj> : compact_serializer
{
