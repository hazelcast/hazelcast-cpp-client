...
TEST_F(ClientConfigTest, testSetGetLogHandler)
{
    ...
    auto h = [&called](const std::string&, const std::string&, logger::level, const std::string&) {
        called = true;
    };
...
TEST_F(ClientConfigTest, test_set_instance_name)
{
    ...
    hazelcast_client client(
      new_client(std::move(client_config().set_instance_name(test_name))).get());
...
{
    ASSERT_THROW(
      client_config().get_connection_strategy_config().get_retry_config().set_jitter(1.01),
...
TEST_F(connection_retry_config_test, max_backoff_duration_boundaries)
{
    ...
    ASSERT_THROW(retry_config.set_max_backoff_duration(std::chrono::milliseconds(-1)),
                 exception::illegal_argument);
...
{
    auto retry_config = client_config().get_connection_strategy_config().get_retry_config();
    ASSERT_THROW(retry_config.set_initial_backoff_duration(std::chrono::milliseconds(-1)),
                 exception::illegal_argument);
...
{
    auto retry_config = client_config().get_connection_strategy_config().get_retry_config();
    ASSERT_THROW(retry_config.set_cluster_connect_timeout(std::chrono::milliseconds(-1)),
                 exception::illegal_argument);
...
TEST_F(ConfiguredBehaviourTest, testAsyncStartTrueNoCluster)
{
    ...
    ASSERT_THROW((client.get_map(random_map_name()).get()), exception::hazelcast_client_offline);
...
TEST_F(ClientSerializationTest, object_data_input_output)
{
    ...
    serialization::object_data_input in(boost::endian::order::big,
                                        out.to_byte_array(),
                                        0,
...
protected:
    ...
    void SetUp() override
    {
        server_.reset(new HazelcastServer(*(boost::endian::order::little == GetParam()
                                              ? little_endian_server_factory_
                                              : g_srvFactory)));

        auto config = get_config();
        config.set_cluster_name(boost::endian::order::little == GetParam() ? "little-endian-cluster"
                                                                           : "dev");
...
protected:
    ...
    {
        auto script =
          (boost::format("var map = instance_0.getMap(\"serialization_with_server_map\");\n"
                         "map.set(\"key\", %1%);\n") %
           object)
            .str();
...
{
    ASSERT_TRUE(
      set_on_server(std::string("new java.lang.Long(") + std::to_string(1LL << 63) + ")"));
...
TEST_P(serialization_with_server, test_int64_array_from_server)
{
    ...
    ASSERT_EQ(std::vector<int64_t>({ 1152921504606846976LL, -1152921504606846976LL }),
...
TEST_P(serialization_with_server, test_string_array_from_server)
{
    ASSERT_TRUE(set_on_server(
      u8"Java.to([\"hey\", \"1⚐中💦2😭‍🙆😔5\"], \"java.lang.String[]\")"));
    ASSERT_EQ(std::vector<std::string>({ u8"hey", u8"1⚐中💦2😭‍🙆😔5" }),
