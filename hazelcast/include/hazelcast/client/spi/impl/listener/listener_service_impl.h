...
{
public:
    listener_service_impl(ClientContext& client_context,
...
private:
    /**
     * Keeps the information related to to an event registration made by
     * clients.
     */
    ...
    struct listener_registration
    {
        ...
        std::shared_ptr<client::impl::BaseEventHandler> handler;
        util::SynchronizedMap<std::shared_ptr<connection::Connection>,
    ...
    void connection_added_internal(
      const std::shared_ptr<connection::Connection>& connection);

    void connection_removed_internal(
      const std::shared_ptr<connection::Connection>& connection);
    ...
    std::vector<boost::asio::strand<boost::asio::thread_pool::executor_type>>
      event_strands_;
