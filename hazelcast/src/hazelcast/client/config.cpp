...
ssl_config::set_context(boost::asio::ssl::context ctx)
{
    ...
    if (enabled_) {
        throw exception::illegal_argument(
          "You should either use the deprecated methods or this method to enable ssl. You already "
          "used the deprecated way.");
    }
...
{
    util::Preconditions::check_not_negative(duration.count(), "duration must be nonnegative");
...
client_config::get_reliable_topic_config(const std::string& name)
{
    ...
    return reliable_topic_config_map_.emplace(name, config::reliable_topic_config{ name })
      .first->second;
}
