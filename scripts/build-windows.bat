...
cmake .. -G %SOLUTIONTYPE% -A %BUILDFORPLATFORM% -DCMAKE_CONFIGURATION_TYPES=%HZ_BUILD_TYPE% -DHZ_LIB_TYPE=%HZ_LIB_TYPE% -DHZ_BIT=%HZ_BIT_VERSION% -DCMAKE_BUILD_TYPE=%HZ_BUILD_TYPE% -DHZ_BUILD_TESTS=ON -DBUILD_GMOCK=OFF -DHZ_BUILD_EXAMPLES=ON -DOPENSSL_ROOT_DIR=%OPENSSL_ROOT_DIR% -DHZ_COMPILE_WITH_SSL=%HZ_COMPILE_WITH_SSL% -DBUILD_GMOCK=OFF -DINSTALL_GTEST=OFF || exit /b 1
