                                     ...
                                     const std::string& object_name)
{
    ...
    auto is_sessionless =
      client::spi::impl::ClientInvocation::create(context_, request, object_name)
        ->invoke()
        .get()
...
raft_proxy_factory::get_group_id(const std::string& proxy_name, const std::string& object_name)
{
    ...
    return client::spi::impl::ClientInvocation::create(context_, request, object_name)
      ->invoke()
      .then(boost::launch::sync, [](boost::future<client::protocol::ClientMessage> f) {
...
boost::future<bool>
atomic_reference::contains_data(const client::serialization::pimpl::data& value_data)
{
...
boost::future<void>
atomic_reference::alter_data(const client::serialization::pimpl::data& function_data)
{
...
latch::try_wait()
{
    return get_count().then(boost::launch::sync,
...
fenced_lock::try_lock()
{
    return try_lock_and_get_fence().then(
      boost::launch::sync, [](boost::future<int64_t> f) { return f.get() != INVALID_FENCE; });
}
...
fenced_lock::try_lock_and_get_fence(std::chrono::milliseconds timeout)
{
    ...
    return do_try_lock_once().then(boost::launch::sync,
...
counting_semaphore::init(int32_t permits)
{
    ...
    return client::spi::impl::ClientInvocation::create(context_, request, object_name_)
      ->invoke()
      .then(boost::launch::sync, [](boost::future<client::protocol::ClientMessage> f) {
...
session_semaphore::throw_illegal_state_exception(std::exception_ptr e)
{
    auto ise = boost::enable_current_exception(
      client::exception::illegal_state("session_semaphore::illegal_state", "No valid session!"));
    if (!e) {
        throw ise;
    }
    try {
