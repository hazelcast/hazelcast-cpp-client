...
@SET EXECUTABLE_NAME=clientTest.exe
...
cmake .. -G %SOLUTIONTYPE% -A %BUILDFORPLATFORM% ^
      ...
      -DCMAKE_CONFIGURATION_TYPES=%HZ_BUILD_TYPE% -DHZ_LIB_TYPE=%HZ_LIB_TYPE% ^
      -DCMAKE_BUILD_TYPE=%HZ_BUILD_TYPE% -DHZ_BUILD_TESTS=ON -DBUILD_GMOCK=OFF ^
      -DHZ_BUILD_EXAMPLES=ON -DHZ_COMPILE_WITH_SSL=%HZ_COMPILE_WITH_SSL% -DINSTALL_GTEST=OFF || exit /b 1
