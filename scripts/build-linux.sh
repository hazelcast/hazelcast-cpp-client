...
cmake .. -DHZ_LIB_TYPE=${HZ_LIB_TYPE} -DHZ_BIT=${HZ_BIT_VERSION} -DCMAKE_BUILD_TYPE=${HZ_BUILD_TYPE} ${HZ_COVERAGE_STRING} -DHZ_BUILD_TESTS=ON -DHZ_BUILD_EXAMPLES=ON -DHZ_COMPILE_WITH_SSL=${HZ_COMPILE_WITH_SSL} -DBUILD_GMOCK=OFF -DINSTALL_GTEST=OFF -DCMAKE_CXX_FLAGS="-m$HZ_BIT_VERSION"
if [ $? -ne 0 ]; then
    echo "Cmake failed !"
    exit 1
fi
