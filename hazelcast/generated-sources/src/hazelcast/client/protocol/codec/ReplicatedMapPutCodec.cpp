...
namespace hazelcast {
    namespace client {
        namespace protocol {
            namespace codec {
                const ReplicatedMapMessageType ReplicatedMapPutCodec::RequestParameters::TYPE = HZ_REPLICATEDMAP_PUT;
                const bool ReplicatedMapPutCodec::RequestParameters::RETRYABLE = false;
                const int32_t ReplicatedMapPutCodec::ResponseParameters::TYPE = 105;
                std::auto_ptr<ClientMessage> ReplicatedMapPutCodec::RequestParameters::encode(
                    ...
                    clientMessage->setMessageType((uint16_t)ReplicatedMapPutCodec::RequestParameters::TYPE);
                ...
                int32_t ReplicatedMapPutCodec::RequestParameters::calculateDataSize(
                        const std::string &name, 
                        const serialization::pimpl::Data &key, 
                        const serialization::pimpl::Data &value, 
                        int64_t ttl) {
                ...
                ReplicatedMapPutCodec::ResponseParameters::ResponseParameters(ClientMessage &clientMessage) {
                    if (TYPE != clientMessage.getMessageType()) {
                        throw exception::UnexpectedMessageTypeException("ReplicatedMapPutCodec::ResponseParameters::decode", clientMessage.getMessageType(), TYPE);
                    }
