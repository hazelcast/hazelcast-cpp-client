...
public:
     ...
     * @return A pair of value and bool. The second part of the pair is true if the insertion of the
     * new values actually happened, i.e. there was no entry associated with the key, otherwise it
     * is set to false. The first part of the pair is existing value or the newly inserted value.
     */
    std::pair<std::shared_ptr<V>, bool> get_or_put_if_absent(const K& key,
