...
RUN deb http://deb.debian.org/debian-security/ buster/updates main
RUN deb http://deb.debian.org/debian buster-updates main
RUN apt-get update && \
    apt-get install -y libssl-dev:i386 wget openssh-server
# install CMake
