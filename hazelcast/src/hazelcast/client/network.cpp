...
ClientConnectionManagerImpl::authenticate_on_cluster(std::shared_ptr<Connection>& connection)
{
    ...
    try {
        ...
        auto response = f.get();
        auto* initial_frame = reinterpret_cast<protocol::ClientMessage::frame_header_t*>(
    ...
    switch (authentication_status) {
        ...
        case protocol::CREDENTIALS_FAILED: {
            auto e = std::make_exception_ptr(exception::authentication(
              ...
              "client_config::set_cluster_name()) does not match the one configured in the cluster "
...
ClientConnectionManagerImpl::connect_to_all_members()
{
    ...
    for (const auto& member : client_.get_client_cluster_service().get_member_list()) {
        ...
        if (client_.get_lifecycle_service().is_running() && !get_connection(member_addr) &&
            ...
            // submit a task for this address only if there is no other pending connection attempt
            // for it
    ...
    do {
        ...
        for (const address& server_address : get_possible_member_addresses()) {
            ...
            if (!tried_addresses_per_attempt.insert(server_address).second) {
                // if we can not add it means that it is already tried to be connected with the
                // member list
                continue;
            }
...
void
ClientConnectionManagerImpl::on_connection_close(const std::shared_ptr<Connection>& connection)
{
...
{
    {
        ...
        if (existing_connection) {
            connection->close(
              (boost::format("Duplicate connection to same member with uuid : %1%") %
        ...
        auto cluster_id_changed =
          !current_cluster_id.is_nil() && !(new_cluster_id == current_cluster_id);
...
ClientConnectionManagerImpl::connect(const address& addr)
{
    ...
    if (!target) {
        BOOST_THROW_EXCEPTION(
          exception::null_pointer("ClientConnectionManagerImpl::connect",
    ...
    HZ_LOG(logger_,
           info,
           boost::str(boost::format("Trying to connect to %1%. Translated address:%2%.") % addr %
                      target));
  ...
  , cluster_connect_timeout_millis_(retry_config.get_cluster_connect_timeout())
{
    if (cluster_connect_timeout_millis_ == std::chrono::milliseconds::max()) {
        ...
        cluster_connect_timeout_text_ =
          (boost::format("%1% msecs") % cluster_connect_timeout_millis_.count()).str();
    }
}
...
wait_strategy::sleep()
{
    ...
    if (time_passed > cluster_connect_timeout_millis_) {
        HZ_LOG(logger_,
               warning,
               (boost::format("Unable to get live cluster connection, cluster connect timeout "
                              "(%1%) is reached. Attempt %2%.") %
    ...
    auto actual_sleep_time =
      ...
      milliseconds(static_cast<milliseconds::rep>(current_backoff_millis_.count() * jitter_ *
                                                  (2.0 * random_(random_generator_) - 1.0)));

    actual_sleep_time =
      (std::min)(actual_sleep_time, cluster_connect_timeout_millis_ - time_passed);

    HZ_LOG(logger_,
           warning,
           (boost::format("Unable to get live cluster connection, retry in %1% ms, attempt: %2% , "
...
SocketFactory::create(const address& address, std::chrono::milliseconds& connect_timeout_in_millis)
{
#ifdef HZ_BUILD_WITH_SSL
    if (ssl_context_.get()) {
        return std::unique_ptr<hazelcast::client::socket>(new internal::socket::SSLSocket(
          io_,
          *ssl_context_,
          address,
    ...
    return std::unique_ptr<hazelcast::client::socket>(new internal::socket::TcpSocket(
      io_,
      address,
      client_context_.get_client_config().get_network_config().get_socket_options(),
      connect_timeout_in_millis,
