...
protected:
                                                          ...
                                                          serialization::pimpl::data* filter_data)
    {
        ...
        util::Preconditions::check_not_negative(
          min_count, (boost::format("min_count (%1%) can't be smaller than 0") % min_count).str());
        util::Preconditions::check_true(
          max_count >= min_count,
          (boost::format("max_count (%1%) should be equal or larger than min_count(%2%)") %
           max_count % min_count)
            .str());
        try {
            capacity().get();
        ...
        util::Preconditions::check_true(
          max_count <= capacity,
          (boost::format(
             "the max_count(%1%) should be smaller than or equal to the capacity(%2%)") %
           max_count % capacity)
            .str());
