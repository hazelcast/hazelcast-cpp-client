...
main()
{
    ...
    /**
     * How a client reconnect to cluster after a disconnect can be configured. The following example
     * configures a total timeout of 30 seconds to connect to a cluster, by initial backoff time
     * being 100 milliseconds and doubling the time before every try with a jitter of 0.8 up to a
     * maximum of 3 seconds backoff between each try.
     */
