 ...
 * not make a call to hazelcast. It can cause deadlock.
 *
...
public:
    ...
    /**
     * Set an handler function to be invoked after a WAN replicated entry is
     * merged. \param h a `void` function object that is callable with a single
     * parameter of type `EntryEvent &&` \return `*this`
     */
    template<typename Handler,
             typename = util::enable_if_rvalue_ref_t<Handler&&>>
    ...
    /**
     * Set an handler function to be invoked when all entries are evicted by
     * imap::evict_all \param h a `void` function object that is callable with a
    ...
    /**
     * Set an handler function to be invoked when all entries are removed by
     * imap::clear \param h a `void` function object that is callable with a
