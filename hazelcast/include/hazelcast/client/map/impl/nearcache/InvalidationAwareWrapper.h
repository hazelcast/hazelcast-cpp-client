        ...
        namespace map {
            namespace impl {
                namespace nearcache {
                    ...
                    public:
                        static hazelcast::util::SharedPtr<internal::nearcache::NearCache<K, V> > asInvalidationAware(
                                hazelcast::util::SharedPtr<internal::nearcache::NearCache<K, V> > nearCache,
                                int markerCount) {
                            return hazelcast::util::SharedPtr<internal::nearcache::NearCache<K, V> >(
                                    new InvalidationAwareWrapper<K, V>(nearCache, markerCount));
                        }

                        InvalidationAwareWrapper(
                                hazelcast::util::SharedPtr<internal::nearcache::NearCache<K, V> > cache,
                                int partitionCount)
                                : nearCache(cache),
                        ...
                        //@Override
                        hazelcast::util::SharedPtr<V> get(const hazelcast::util::SharedPtr<K> &key) {
                            return nearCache->get(key);
                        }

                        //@Override
                        void put(const hazelcast::util::SharedPtr<K> &key, const hazelcast::util::SharedPtr<V> &value) {
                            nearCache->put(key, value);
                        }

                        //@Override
                        void put(const hazelcast::util::SharedPtr<K> &key,
                                 const hazelcast::util::SharedPtr<serialization::pimpl::Data> &value) {
                            nearCache->put(key, value);
                        }

                        //@Override
                        bool remove(const hazelcast::util::SharedPtr<K> &key) {
