...
public:
    ...
    /**
     * Integer constant for representing behaviour for giving higher priority to
     * first entry to be evicted.
     */
    static const int FIRST_ENTRY_HAS_HIGHER_PRIORITY_TO_BE_EVICTED = -1;

    /**
     * Integer constant for representing behaviour for giving higher priority to
     * second entry to be evicted.
     */
    static const int SECOND_ENTRY_HAS_HIGHER_PRIORITY_TO_BE_EVICTED = +1;

    /**
     * Integer constant for representing behaviour for giving same priority to
     * both of entry to be evicted.
     */
    static const int BOTH_OF_ENTRIES_HAVE_SAME_PRIORITY_TO_BE_EVICTED = 0;

    /**
     * Compares the given {@link EvictableEntryView} instances and returns the
     * result. The result should be one of the <ul>
