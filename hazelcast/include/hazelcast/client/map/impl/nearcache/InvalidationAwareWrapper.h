...
namespace nearcache {
/**
 * Aware of invalidations and if an invalidation for a key comes before put to
 * cache, cached value will be removed. See usages of this wrapper in proxies.
 *
 * @see KeyStateMarker
...
{
public:
    static std::shared_ptr<internal::nearcache::NearCache<K, V>>
    as_invalidation_aware(
    ...
    /*
                            ...
                            void put(const std::shared_ptr<K> &key,
                                     const
       std::shared_ptr<serialization::pimpl::data> &value) { nearCache->put(key,
       value);
                            }
    */
