...
main()
{
    ...
    NearCacheSupport::print_near_cache_stats(
      map,
      "The put(1..100, article) calls have no effect on the empty Near Cache");

    for (int i = 1; i <= 100; i++) {
        map->get<int, std::string>(i).get();
    }
    NearCacheSupport::print_near_cache_stats(
      map, "The first get(1..100) calls populate the Near Cache");
    ...
    NearCacheSupport::print_near_cache_stats(
      map,
      "The put(101, \"bar\") call has no effect on the populated Near Cache");

    map->get<int, std::string>(101).get();
    NearCacheSupport::print_near_cache_stats(
      map,
      "The first get(101) call triggers the eviction and population of the "
      "Near Cache");

    NearCacheSupport::wait_for_near_cache_eviction_count(map, 1);
    NearCacheSupport::print_near_cache_stats(map,
                                             "The Near Cache has been evicted");
